<template>
  <label :class="['base-checkbox', { 'is-disabled': disabled, 'is-checked': isChecked }]">
    <span class="base-checkbox__input">
      <input
        type="checkbox"
        class="base-checkbox__original"
        :checked="isChecked"
        :disabled="disabled"
        @change="handleChange"
      />
      <span class="base-checkbox__inner" />
    </span>
    <span v-if="$slots.default" class="base-checkbox__label">
      <slot />
    </span>
  </label>
</template>

<script setup lang="ts">
import { computed } from 'vue';

interface Props {
  modelValue: boolean | unknown[];
  label?: string | number;
  disabled?: boolean;
}

const props = defineProps<Props>();
const emit = defineEmits(['update:modelValue']);

const isChecked = computed(() => {
  if (Array.isArray(props.modelValue)) {
    return props.modelValue.includes(props.label);
  }
  return !!props.modelValue;
});

const handleChange = (event: Event) => {
  if (props.disabled) return;
  const { checked } = event.target as HTMLInputElement;
  if (Array.isArray(props.modelValue)) {
    const newValue = [...props.modelValue];
    if (checked) {
      newValue.push(props.label);
    } else {
      const index = newValue.indexOf(props.label);
      if (index > -1) {
        newValue.splice(index, 1);
      }
    }
    emit('update:modelValue', newValue);
  } else {
    emit('update:modelValue', checked);
  }
};
</script>

<style scoped>
.base-checkbox {
  color: var(--c-text-regular);
  font-weight: 500;
  font-size: 14px;
  position: relative;
  cursor: pointer;
  display: inline-flex;
  align-items: center;
  white-space: nowrap;
  user-select: none;
  margin-right: 30px;
}

.base-checkbox.is-disabled {
  color: var(--c-text-placeholder);
  cursor: not-allowed;
}

.base-checkbox__input {
  white-space: nowrap;
  cursor: pointer;
  outline: none;
  display: inline-flex;
  position: relative;
}

.base-checkbox__original {
  opacity: 0;
  outline: none;
  position: absolute;
  margin: 0;
  width: 0;
  height: 0;
  z-index: -1;
}

.base-checkbox__inner {
  display: inline-block;
  position: relative;
  border: 1px solid var(--c-border-base);
  border-radius: var(--radius-sm);
  box-sizing: border-box;
  width: 14px;
  height: 14px;
  background-color: var(--c-background);
  z-index: 1;
  transition: border-color 0.25s cubic-bezier(0.71, -0.46, 0.29, 1.46),
    background-color 0.25s cubic-bezier(0.71, -0.46, 0.29, 1.46);
}

.base-checkbox__inner:after {
  box-sizing: content-box;
  content: '';
  border: 1px solid #fff;
  border-left: 0;
  border-top: 0;
  height: 7px;
  left: 4px;
  position: absolute;
  top: 1px;
  transform: rotate(45deg) scaleY(0);
  width: 3px;
  transition: transform 0.15s ease-in 0.05s;
  transform-origin: center;
}

.is-checked .base-checkbox__inner {
  background-color: var(--c-primary);
  border-color: var(--c-primary);
}

.is-checked .base-checkbox__inner:after {
  transform: rotate(45deg) scaleY(1);
}

.base-checkbox__label {
  padding-left: 10px;
  line-height: 1;
  font-size: inherit;
}
</style>
