      ...
      try {
          ...
          switch (definition.archetype) {
              ...
              case 'stream-action': {
                  ...
                  // Note: onNodeEnd for stream-action is handled in _handleSubscribable callbacks.
                  break;
              }
              case 'pure': {
                  const outputs = await NodeExecutor.executeNodeLogic(node, inputs);
          ...
          console.error(`[Engine] Error executing node ${node.id}:`, error);
          this.hooks?.onNodeEnd(node.id, 'error');
      }
    }
